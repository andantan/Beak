...
from Core.Errors.exceptions import CommonQueueExceptions, PostfixQueueExceptions

from Tools.Decorator.decorators import QueueInspector

from Data.Cache.settings import POSTFIX_QUEUE_MAX_LENGTH

from Types.Classes import *



class PostfixQueue(metaclass=Singleton):
    __slots__ = (
        "queue",
    )
    ...
    __throwable__: Tuple[str] = (
        ...
        "<exception PostfixQueueExceptions.SaturationOfQueueException>",
    )
    ...
    def enqueue(self, __SI: int, audioizated: List[Audio]) -> None:
        for audio in audioizated:
            __queue = self.queue.__getitem__(__SI)
            
            if POSTFIX_QUEUE_MAX_LENGTH - 1 < len(__queue):
                self.remove(__SI, 0)
    ...
    def refresh(self, __SI: int) -> None:
        ...
        if POSTFIX_QUEUE_MAX_LENGTH < len(self.queue.__getitem__(__SI)):
            self.remove(__SI, 0)
    ...
    def insert(self, __SI: int, index: int, audio: Audio) -> None:
        ...
        if len(__queue) < POSTFIX_QUEUE_MAX_LENGTH:
            __queue.insert(index, audio)
        else:
            raise PostfixQueueExceptions.SaturationOfQueueException
